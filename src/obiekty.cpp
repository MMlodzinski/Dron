...
void Zliczanie::wypisz(){
    std::cout<<"Ilość stworzonych obiektów: "<<stworzono;
    std::cout<<"Ilość istniejacych obiektów: "<<istnieje;
}
...
void Dno::Ustaw(){
    ...
    for(i=0;i<DLUGOSC_DNA+1;i++){
        this->punkty[i][0]= Wektor3D(-szerokosc/2+i,-dlugosc/2,-wysokosc/2);
        this->punkty[i][1]= Wektor3D(-szerokosc/2+i,dlugosc/2,-wysokosc/2);
    }

    for(i=0;i<DLUGOSC_DNA+1;i++){
        this->punkty[i+DLUGOSC_DNA+1][0]= Wektor3D(-szerokosc/2,-dlugosc/2+i,-wysokosc/2);
        this->punkty[i+DLUGOSC_DNA+1][1]= Wektor3D(szerokosc/2,-dlugosc/2+i,-wysokosc/2);
    }
}
...
Dno::Dno(){
    ...
    this->punkty=std::vector<std::vector<drawNS::Point3D>>(2*DLUGOSC_DNA+2,std::vector<drawNS::Point3D>(2,drawNS::Point3D(0,0,-wysokosc/2)));
}
...
PowierzchniaWody::PowierzchniaWody(){
    ...
    this->dlugosc=DLUGOSC_DNA;
    this->wysokosc=DLUGOSC_DNA;
    this->punkty=std::vector<std::vector<drawNS::Point3D>>(DLUGOSC_DNA+1,std::vector<drawNS::Point3D>(DLUGOSC_DNA+1,drawNS::Point3D(0,0,wysokosc/2)));
}
...
double PrzeszkodaProst::promien(){
    return (Wektor3D(this->dlugosc, this->szerokosc, this->wysokosc).dlugosc()/2);
}

bool PrzeszkodaProst::czyKolizja(std::shared_ptr<InterfejsDrona> dron){
    return (this->promien()+dron->promien()>=(this->pozycjaSrodka-dron->pozycja()).dlugosc());
}

bool Dno::czyKolizja(std::shared_ptr<InterfejsDrona> dron){
    return (-(this->wysokosc/2)+dron->promien()<=dron->pozycja()[2]);
}

bool PowierzchniaWody::czyKolizja(std::shared_ptr<InterfejsDrona> dron){
    return (dron->pozycja()[2]>=this->wysokosc/2;
}